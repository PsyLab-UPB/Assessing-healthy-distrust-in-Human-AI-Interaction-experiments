 /**
 * @title HeDi-TOJ Exp 2 First Part
 * @description HeDi-TOJ Exp2 First Part - Lab-version 25-06-04
 * @version 2.0.0
 *
 * @assets assets/
...
export async function run({ assetPaths, input = {}, environment, title, version }) {
  ...
  const procedure = `
  <p>Ihre Aufgabe in diesem Teil des Experiments ist es, zu lernen, die hier verwendeten Formen einem von zwei Typen richtig zuzuordnen.</p>
        ...
        questions: [
          {
            ...
            options: [
                "Ja, an der ersten Studien zur Warhnehmung von Bild-Klassifikationen",
                "Ja, an der Studie zu geometrischen Formen mit KI-Unterstützung",
                "Ja, an beiden der genannten Studien",
                "Nein"],
            required: true,
          },
        ],
  ...
  */
  ...
  for (let i = 0; i < trialSetFullLength.length; i++) {
    ...
    timeline.push(
      ...
      modification_function: (element) => ImageTojPluginTryOut.flashElement(element, "toj-flash", 30),
      play_feedback: false,
      //probeLeft: jsPsych.timelineVariable("probeLeft", true),
      on_start: (tmp) => {
        const probeLeft = trial.probeLeft;        // Log probeLeft
        const clssfctnCorrect = trial.clssfctnCorrectness;
        const clssfctnType = trial.clssfctnType;
