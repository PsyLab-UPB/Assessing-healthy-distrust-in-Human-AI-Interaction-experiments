 ...
 * @license MIT
 */
export class ImageTojPluginTryOut extends TojPlugin {
  static info = {
    name: "toj-image",
    parameters: {
  ...
  /**
   * The ImageTojPluginTryOut instance of the current trial (if a trial is currently running)
   * @type {ImageTojPluginTryOut|null}
   */
  static current = null;
  ...
  static createImageElement(imageSrc, cssClass, imageProperties = {}, hidden) {
    ...
    if (hidden) {
      ...
      setTimeout (() => {TojPlugin.showElement(image)}, 700); // to show fixation-letter on its own for specified duration (700ms)// 100ms for Screentest 
      //setTimeout (() => {TojPlugin.showElement(image)}, 100);
    }
  ...
  addBackgroundImage(imageSrc, imageProperties) {
    const image = ImageTojPluginTryOut.createImageElement(imageSrc, "toj-background", imageProperties);
  ...
  async trial(display_element, trial, on_load) {
    ImageTojPluginTryOut.current = this;  
    this._appendContainerToDisplayElement(display_element, trial);  
    
    // Create stimulus image elements
    trial.probe_element = ImageTojPluginTryOut.createImageElement(
      trial.probe_image,
      "toj-probe",
      trial.probe_properties,
      trial.hide_stimuli
    );
    trial.reference_element = ImageTojPluginTryOut.createImageElement(
    ...
    // tryOut
    const clssfct = document.createElement("html");
    clssfct.innerHTML = `KI-Klassifikation: `+trial.clssfctnType+``;
    clssfct.style.marginTop = "-200px";
    ...
    // tryOut comment in to work here
    //this.appendElement(clssfct);

    on_load();

    await super.trial(display_element, trial, on_load, false);
    ImageTojPluginTryOut.current = null;
  }
}

export default ImageTojPluginTryOut;
