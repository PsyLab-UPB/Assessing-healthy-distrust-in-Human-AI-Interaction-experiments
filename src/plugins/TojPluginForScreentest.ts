...
const info = <const>{
  ...
  parameters: {
    ...
    /**
     * [optional] A promise to inject other input modalities (e.g. touch, speech recognition)
     */
     customAwait: {
      type: ParameterType.OBJECT,
      default: null,
...
export class TojPlugin implements JsPsychPlugin<Info> {
  ...
  async trial(display_element, trial: TrialType<Info>, on_load, standalone = true) {
    ...
    let customAwait = trial.customAwait;

    if(!customAwait){
      customAwait = new Promise((resolve) => {})
    }
